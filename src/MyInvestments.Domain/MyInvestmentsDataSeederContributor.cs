    ...
    : IDataSeedContributor, ITransientDependency
{
    ...
    public async Task SeedAsync(DataSeedContext context)
    {
        ...
        if (await _setorRepository.GetCountAsync() != 0)
        {
            ...
            {
                throw new Exception("ERROR 001: One or more related classeAtivo ou Setor were not found.");
            }
            ...
            {
                throw new Exception("ERROR 002: One or more related ativo na operacao were not found.");
            }

            var tipoTransacao = await _tipoTransacaoRepository.GetListAsync();

            var venda = tipoTransacao.FirstOrDefault(s => s.Descricao == "Venda")?.Id ?? Guid.Empty;
            var compra = tipoTransacao.FirstOrDefault(s => s.Descricao == "Compra")?.Id ?? Guid.Empty;

            if (venda == Guid.Empty || compra == Guid.Empty)
            {
                throw new Exception("ERROR 003: One or more related tipo Trasacao na operacao were not found.");
            }
